...
@pytest.mark.asyncio
async def test_rs_method_execute(
    hass: HomeAssistant, rs_domain_switch: RsDomain, caplog
):
    method = rs_domain_switch.methods["rs_turn_on"]
    ...
    with caplog.at_level(logging.WARNING):
        ...
        with patch.object(hass.services, "async_call") as async_call:
            with patch.object(method, "_async_wait_template") as _async_wait_template:
                ...
                assert (
                    "call-entities ['switch.switch_1', 'switch.switch_2']"
...
@pytest.mark.asyncio
async def test_rs_method_async_wait_template(
    hass: HomeAssistant, rs_domain_climate: RsDomain, caplog
):
    ...
    with patch(
        ...
        with patch("custom_components.rs.rs_method.async_template") as async_template:
            ...
            assert (
                template.template
                == "{{state_attr('climate.thermo_1', 'temperature')|float(0) == temperature|float}}"
            )
            variables = async_template.mock_calls[0].args[2]
            assert len(variables) == 1
            assert variables["temperature"] == 70
            assert async_template.mock_calls[0].args[3] == False
            ...
            assert (
                template.template
                == "{{state_attr('climate.thermo_2', 'temperature')|float(0) == temperature|float}}"
            )
            variables = async_template.mock_calls[1].args[2]
            assert len(variables) == 1
            assert variables["temperature"] == 70
            assert async_template.mock_calls[1].args[3] == False
...
class TemplateMock:
    ...
    def my_async_track_template(
        self, hass: HomeAssistant, template: Template, action, variables
    ):
...
async def test_rs_method_async_wait_template_1(
    hass: HomeAssistant, rs_domain_climate: RsDomain, caplog
):
    method = rs_domain_climate.methods["rs_set_temperature"]
    call_data = {
    ...
    with patch(
        ...
        with patch(
            ...
            res = await hass.async_create_task(
                method._async_wait_template(call.data, 5)
            )
    ...
    with patch(
        ...
        with patch(
            ...
            async_template.side_effect = [True, False]
            res = await hass.async_create_task(
                method._async_wait_template(call.data, 1)
            )
            assert mock.my_async_track_template_count == 1
            assert mock.my_unsub_count == 1
            assert (
    ...
    with patch(
        ...
        with patch(
            ...
            assert variables["temperature"] == 70
            assert variables["hvac_mode"] == "heat"
            assert async_template.mock_calls[2].args[3] == False
            ...
            assert variables["temperature"] == 70
            assert variables["hvac_mode"] == "heat"
            assert async_template.mock_calls[3].args[3] == False
